...
def load_data(tickers, days=1095): 
...
def calculate_quant_metrics(df_vnindex, df_stocks, window_size): 
    market_ret = df_vnindex['VNINDEX'].pct_change()
    stocks_ret = df_stocks.pct_change()
    stocks_ret = stocks_ret.reindex(market_ret.index)
    
    # Loại bỏ NA và kiểm tra độ dài dữ liệu
    market_ret_dropna = market_ret.dropna()
    if market_ret_dropna.empty or len(market_ret_dropna) < window_size:
        st.error(f"❌ Lỗi: Dữ liệu VN-Index quá ngắn.")
        st.stop()
        
    # Ép kiểu dữ liệu để EGARCH không báo lỗi Cython
    ...
    try:
        ...
        st.error(f"❌ Lỗi EGARCH: {e}")
        st.stop()
        
    # 2. Skewness dùng Rolling (Lấy phương hướng đối xứng +- qua 0)
    rolling_skew = market_ret_dropna.rolling(window=window_size).skew().ffill()
    
    # 3. Correlation dùng EWM (Giữ cấu trúc mượt mà, truyền window vào halflife)
    ...
    downside_corr = stock_down.ewm(halflife=window_size, min_periods=15).corr(market_down).median(axis=1).ffill()
    upside_corr = stock_up.ewm(halflife=window_size, min_periods=15).corr(market_up).median(axis=1).ffill()
    
    # 4. CSV Index
    roll_var_m = market_ret_dropna.rolling(window=window_size).var()
    roll_cov = stocks_ret.rolling(window=window_size).cov(market_ret_dropna)
    roll_beta = roll_cov.div(roll_var_m, axis=0)
    roll_var_i = stocks_ret.rolling(window=window_size).var()
...
# THANH TRƯỢT NẰM Ở TRÊN TRƯỚC KHI TRUYỀN VÀO HÀM
lookback_window = st.sidebar.slider(
    "⏳ Khung thời gian rủi ro (Window)", 
    min_value=20, 
    max_value=60, 
    value=60, 
    step=10,
    help="20: Ngắn hạn (1 tháng) | 60: Trung hạn (1 quý). Áp dụng cho Skewness, CSV và Halflife của Correlation."
)
...
if df_vnindex is not None and df_stocks is not None:
    # TRUYỀN BIẾN WINDOW_SIZE VÀO ĐÂY
    metrics_df = calculate_quant_metrics(df_vnindex, df_stocks, window_size=lookback_window)
    ...
    fig.update_layout(height=850, hovermode="x unified", margin=dict(l=20, r=20, t=40, b=20), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
